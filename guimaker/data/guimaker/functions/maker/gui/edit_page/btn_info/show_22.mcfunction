#> Slot 22 buton bilgisi — @s = editing_page oyuncu
#> Marker'dan Slot 22'nin BUTTON_INFO'sunu çek
data remove storage guimaker:temp btn_view
data modify storage guimaker:temp btn_view set from entity @e[type=marker,tag=gm.itemcheck,tag=gm.gui.block,limit=1,sort=nearest] data.PAGE.INIT[{Slot:22b}].tag.BUTTON_INFO

tellraw @s {"text":"-------------------------------","color":"#FFDF8D"}
tellraw @s ["",{"text":"[GUI Maker] ","bold":true,"color":"#3FFEFE"},{"text":"Slot 22 Buton Ayarı","color":"white"}]
...
tellraw @s {"text":"-------------------------------","color":"#FFDF8D"}
scoreboard players set $target_slot gm.Values 22
