#> GUI[12] seç — sayfa listesini göster (barrel yerleştirme için)
scoreboard players set $found_gui_idx gm.Values 12
execute store result score $page_count gm.Values run data get storage barden:gui.all GUI[12].PAGES
data remove storage guimaker:temp pages_list
execute if score $page_count gm.Values matches 1.. run data modify storage guimaker:temp pages_list set from storage barden:gui.all GUI[12].PAGES
