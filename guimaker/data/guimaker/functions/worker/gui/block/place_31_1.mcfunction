#> Place GUI[31] PAGES[1] barrel — @s = oyuncu
...
data modify entity @e[type=marker,tag=gm.gui.container.place,sort=nearest,limit=1] data.page_number set from storage barden:gui.all GUI[31].PAGES[1].page_number
